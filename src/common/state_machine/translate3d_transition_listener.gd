...
@export var position_other: Node3D
...
func _ready() -> void:
	...
	assert(position_other != null, Errors.NULL_NODE)
...
func _on_target_state_exited(data: Dictionary) -> void:
	...
	tween.set_trans(Tween.TRANS_SINE)
	tween.set_ease(Tween.EASE_IN_OUT)
	tween.tween_property(target_node, "global_position", position_other.global_position, tween_duration)
