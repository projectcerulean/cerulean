...
const END_TIME_SECONDS: float = 1.0
const EXPECTED_LOG_SLOPE: float = log(pow(10.0, -LERP_WEIGHT))

const test_params: Array = [
	[1.0 / 60.0],
	[1.0 / 120.0],
	[1.0 / 240.0],
	[1.0 / 360.0],
...
func test_delta_lerp(params: Array = use_parameters(test_params)) -> void:
	...
	for i: int in range(len(timestamps)):
