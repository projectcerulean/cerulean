...
func test_scene_transition() -> void:
	...
	for i in range(3):
		for test_scene_path in test_scene_paths:
			...
			# Check scene info
			assert_eq(scene_info_resource.scene_path, test_scene_path)
			assert_eq(scene_info_resource.spawn_point_id, i)

			# Check player position
			var test_scene: Node = get_test_scene()
			var player_global_position: Vector3 = player_transform_resource.value.origin
			assert_ne(player_global_position, Vector3.ZERO)
			...
			assert_not_null(closest_spawn_point_to_player)
			assert_eq(closest_spawn_point_to_player.id, i)

			# Check player rotation
			var player_rotation_euler: Vector3 = player_transform_resource.value.basis.get_euler()
			assert_ne(player_rotation_euler, Vector3.ZERO)
			assert_almost_eq(
			...
			assert_ne(camera_global_position, Vector3.ZERO)
