...
func test_scene_transition() -> void:
	...
	for i in range(3):
		for test_scene_path in test_scene_paths:
			...
			# Check scene info
			assert_eq(scene_info_resource.get_scene_path(), test_scene_path, "Incorrect scene path in scene data resource after scene transition to %s, spawn point %s" % [test_scene_path, i])
			assert_eq(scene_info_resource.get_spawn_point_id(), i, "Incorrect spawn point id in scene data resource after scene transition to (%s, %s)" % [test_scene_path, i])

			# Check player position
			var test_scene: Scene = get_test_scene()
			var player_global_position: Vector3 = player_transform_resource.get_value().origin
			...
			var player_rotation_euler: Vector3 = player_transform_resource.get_value().basis.get_euler()
			...
			var camera_global_position: Vector3 = camera_transform_resource.get_value().origin
			...
			assert_almost_eq(
				-player_transform_resource.get_value().basis.z,
