...
func emit_state_entered(sender: Node, state: Node) -> void:
	call_deferred("emit_signal", state_entered.get_name(), sender, state)


func emit_state_exited(sender: Node, state: Node) -> void:
	call_deferred("emit_signal", state_exited.get_name(), sender, state)
