...
func physics_process(delta: float) -> void:
	...
	var water_surface_height: float = player.water_detector.get_water_surface_height()
	if surfaced:
		velocity_y = 0.0
		if not is_inf(water_surface_height):
			player.global_transform.origin.y = water_surface_height
	else:
		...
		if not is_inf(water_surface_height) and player.velocity.y > 0.0 and player.global_transform.origin.y > player.water_detector.get_water_surface_height():
			surfaced = true
...
func get_transition() -> StringName:
	if not player.water_detector.is_in_water() and player.are_raycasts_colliding():
