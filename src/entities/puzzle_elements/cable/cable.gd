...
@onready var flow_position_start: float = -box_mesh.size.y / 4.0 - shader_material.get_shader_uniform("smooth_range") / 2.0
...
func _ready() -> void:
	...
	shader_material.set_shader_uniform("flip_colors", state_next)
	set_flow_position(flow_position_start)


func set_flow_position(flow_position: float) -> void:
	shader_material.set_shader_uniform("flow_position", flow_position)
...
func _on_state_entered(sender: Node, state: StringName, _data: Dictionary) -> void:
	if sender.owner == input_node:
		...
		shader_material.set_shader_uniform("flip_colors", flip_colors)
		set_flow_position(flow_position_start)
